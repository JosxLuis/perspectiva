...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    ...
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."libro WHERE id".DB_PREFIJO."libro= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."libro WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."libro WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."libro
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."libro WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."libro  ORDER BY id".DB_PREFIJO."libro DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
...
<head>
	<meta charset="UTF-8">
	<title>Libreria - <?php echo PROYECTO; ?></title>
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				...
	        				?>
			        			<tr>
			        				...
			        				<td><?php echo utf8_encode($rowPublicacion['nombre']); ?></td>
			        				<td><?php echo utf8_encode($rowPublicacion['cantidad']); ?></td>
			        				<td><?php echo utf8_encode(mostrarNombre($rowPublicacion['id'.DB_PREFIJO.'libro_categoria'],"libro_categoria","titulo")); ?></td>
			        				<td><?php echo utf8_encode($rowPublicacion['creado']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'libro']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'libro']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
