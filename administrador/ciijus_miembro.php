...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    ...
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."miembro WHERE id".DB_PREFIJO."miembro= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."miembro WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."miembro WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."miembro
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."miembro WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."miembro  ORDER BY id".DB_PREFIJO."miembro DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				...
	        				?>
			        			<tr>
			        				...
			        				<td><?php echo utf8_encode($rowPublicacion['puesto']); ?></td>
			        				<td><?php echo utf8_encode($rowPublicacion['origen']); ?></td>
			        				<td><?php echo utf8_encode($rowPublicacion['creado']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'miembro']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'miembro']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
