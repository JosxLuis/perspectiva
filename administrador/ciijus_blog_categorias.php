...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."post_categoria WHERE id".DB_PREFIJO."post_categoria= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."post_categoria WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."post_categoria  ORDER BY id".DB_PREFIJO."post_categoria DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
...
<head>
	<meta charset="UTF-8">
	<title>Blog - categorias - <?php echo PROYECTO; ?></title>
	        	...
	        	<table class="regular">
	        		...
	        		<tbody>
	        			<?php if($cantidad != 0){ ?>
	        				...
	        				?>
			        			<tr>
			        				<td align="center">#<?php echo $rowCategoria['id'.PREFIJO.'post_categoria']; ?></td>
			        				...
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowCategoria['id'.DB_PREFIJO.'post_categoria']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowCategoria['id'.DB_PREFIJO.'post_categoria']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
