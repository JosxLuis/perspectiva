<?php
	...
	if(isset($_POST['editar']) && $_POST['usuario'] != ""){
		...
		$editarPublicacion = "UPDATE ".DB_PREFIJO."administrador SET $portada id".DB_PREFIJO."administrador_rol=".$_POST['tipoUsuario'].", correo='".utf8_decode($_POST['correo'])."', nombre='".utf8_decode($_POST['nombre'])."' , apellidos='".utf8_decode($_POST['apellidos'])."', telefono='".utf8_decode($_POST['telefono'])."' 
					...
					<div class="form-add">
			        	<form name="form1" action="" method="post" enctype="multipart/form-data">
							...
							<select name="tipoUsuario" id="" required="">
								<option value="<?php echo $consulta['id'.DB_PREFIJO.'administrador_rol']; ?>"><?php echo mostrarNombre($consulta['id'.DB_PREFIJO.'administrador_rol'],"administrador_rol","tipo"); ?></option>
			        		...
			        		<input type="file" name="imagen" <?php if($consulta['avatar'] != ""){ ?> <?php }else{ ?> required <?php } ?>><br>
							<?php if($consulta['avatar'] != ""){ ?> <div class="mintext">Si no selecionas una nueva foto se conserva la anterior</div> <?php }else{ ?>  <?php } ?><br>
