...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    ...
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."oferta WHERE id".DB_PREFIJO."oferta= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."oferta WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."oferta WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."oferta
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."oferta WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."oferta  ORDER BY id".DB_PREFIJO."oferta DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
			...
			<div class="sixteen columns">
				<div class="list-items">
					<div class="titulo">
						...
						<p>Lista de todas los ofertas académicas</p>
					</div>
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				...
	        				?>
			        			<tr>
			        				...
			        				<td><?php echo utf8_encode(mostrarNombre($rowPublicacion['id'.DB_PREFIJO.'oferta_categoria'],"oferta_categoria","titulo")); ?></td>
			        				<td><?php echo utf8_encode($rowPublicacion['creado']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'oferta']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'oferta']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
