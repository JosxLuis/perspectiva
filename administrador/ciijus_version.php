...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."versiones WHERE id".DB_PREFIJO."versiones= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."versiones WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."versiones WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."versiones
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."versiones WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."versiones  ORDER BY id".DB_PREFIJO."versiones DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
	        	...
	        	<table class="regular">
	        		<thead>
	        			<tr>
	        				...
	        				<td width="20%">versiones</td>
	        				<td width="40%">Introduccion</td>
	        		...
	        		<tbody>
	        			<?php if($cantidad != 0){ ?>
	        				<?php 
	        					while($rowVersiones = mysqli_fetch_array($resultado)){
	        						//$tipo = devolverValorQuery("SELECT tipo FROM ".DB_PREFIJO."servicios_tipo WHERE id".DB_PREFIJO."servicios_tipo =".$rowVersiones['id'.DB_PREFIJO.'servicios_tipo']" ");
	        				?>
			        			<tr>
			        				
			        				<td><?php echo utf8_encode($rowVersiones['version']); ?></td>
			        				<td><?php echo utf8_encode($rowVersiones['introduccion']); ?></td>
			        				<td><?php echo utf8_encode($rowVersiones['fecha']); ?></td>
			        				<td><?php echo utf8_encode($rowVersiones['creado']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowVersiones['id'.DB_PREFIJO.'versiones']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowVersiones['id'.DB_PREFIJO.'versiones']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
