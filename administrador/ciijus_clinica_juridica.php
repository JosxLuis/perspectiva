...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
	$portada = devolverValorQuery("SELECT portada FROM ".DB_PREFIJO."clinica WHERE id".DB_PREFIJO."clinica=".$_GET['id']." ");
	if($portada['portada'] != ""){
		unlink("../".$portada['portada']);
	}
	
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."clinica WHERE id".DB_PREFIJO."clinica= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."clinica WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."clinica WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."clinica
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."clinica WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."clinica  ORDER BY id".DB_PREFIJO."clinica DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
		...
		<div class="container">
			<div class="sixteen columns">
				<div class="list-items">
					<div class="titulo">
						<h4>Clinica Juridica</h4>
						<p>Lista de todas las clinicas juridicas</p>
					</div>
	        	...
	        	<table class="regular">
	        		...
	        		<tbody>
	        			<?php if($cantidad != 0){ ?>
	        				<?php 
	        					while($rowClinica = mysqli_fetch_array($resultado)){
	        				...
	        				?>
			        			<tr>
			        				<td><div class="fotografia" style="background:url(<?php echo URL.$rowClinica['portada']; ?>) #e6e6e6;"></div></td>
			        				<td><?php echo utf8_encode($rowClinica['titulo']); ?></td>
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowClinica['id'.DB_PREFIJO.'clinica']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowClinica['id'.DB_PREFIJO.'clinica']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
