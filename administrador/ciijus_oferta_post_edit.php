<?php
	if(isset($_POST['eliminar-foto']) && $_POST['eliminar-foto'] != ""){
		$portada = devolverValorQuery("SELECT portada FROM ".DB_PREFIJO."oferta WHERE id".DB_PREFIJO."oferta=".$_GET['id']." ");
		if($portada['portada'] != ""){
    		unlink("../".$portada['portada']);
		}
		$borrarEntrada = "UPDATE ".DB_PREFIJO."oferta SET portada = '' WHERE id".DB_PREFIJO."oferta= ".$_GET['id']."";
	...
	if(isset($_POST['titulo']) && $_POST['titulo'] != ""){
		...
		if($_POST['fechafin'] != ""){
			$valueFechaFin = ",fechafin='".utf8_decode($_POST['fechafin'])."'";
		}else{
			$valueFechaFin = "";
		}
	
		$editarRegistro = "UPDATE ".DB_PREFIJO."oferta SET $fotografia id".DB_PREFIJO."oferta_categoria=".$_POST['categoria'].",titulo='".utf8_decode(addslashes($_POST['titulo']))."',introduccion='".utf8_decode(addslashes($_POST['introduccion']))."',descripcion='".utf8_decode(addslashes($_POST['descripcion']))."',ubicacion='".utf8_decode(addslashes($_POST['ubicacion']))."',ponente='".utf8_decode(addslashes($_POST['ponente']))."',fechainicio='".utf8_decode($_POST['fechainicio'])."' $valueFechaFin,activo='".utf8_decode($_POST['activo'])."',costo='".utf8_decode($_POST['costo'])."', descuento='".utf8_decode($_POST['descuento'])."' $valueCostoDescuento  WHERE id".DB_PREFIJO."oferta=".$_GET['id']." ";
	...
	$consulta = devolverValorQuery("SELECT * FROM ".DB_PREFIJO."oferta WHERE id".DB_PREFIJO."oferta=".$_GET['id']." ");
			        ...
			        <div class="form-add">
			        	<form name="form1" action="" method="post" enctype="multipart/form-data">
			        		...
			        		<select name="categoria" id="marca" required>
								<option value="<?php echo $consulta['id'.DB_PREFIJO.'oferta_categoria']; ?>"><?php echo utf8_encode(mostrarNombre($consulta['id'.DB_PREFIJO.'oferta_categoria'],"oferta_categoria","titulo")); ?></option>
			        		...
			        		<textarea  name="ubicacion" id="editor2" placeholder="Ubicación"><?php echo utf8_encode($consulta['ubicacion']); ?></textarea>
			    			<label for="ponente">Ponente <span>*</span></label>
			        		<input type="text" name="ponente" placeholder="Ponente de la oferta" value="<?php echo utf8_encode($consulta['ponente']); ?>">
			    			<label for="fechaInicio">Iniciado</label>
			        		<input type="text" name="fechainicio" readonly="readonly" placeholder="YYYY-MM-DD" value="<?php echo $consulta['fechainicio']; ?>" required class="default_datetimepicker">
			        		<label for="fechaFin">Finalizado</label>
			        		<input type="text" name="fechafin" readonly="readonly" placeholder="YYYY-MM-DD" value="<?php echo $consulta['fechafin']; ?>" class="default_datetimepicker">
			        		...
			        		<input type="text" name="costo" placeholder="Costo de la oferta" value="<?php echo utf8_encode($consulta['costo']); ?>" required>
			        		...
			        		<input type="text" name="costo_descuento" placeholder="Costo con el descuento del libro" value="<?php echo utf8_encode($consulta['costo_descuento']); ?>">
