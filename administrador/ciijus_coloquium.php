...
if(isset($_GET['act']) && $_GET['act'] == "eliminar"){
	$portada = devolverValorQuery("SELECT portada FROM ".DB_PREFIJO."coloquium WHERE id".DB_PREFIJO."coloquium=".$_GET['id']." ");
	if($portada['portada'] != ""){
		unlink("../".$portada['portada']);
	}
	
    $borrarEntrada = "DELETE  FROM ".DB_PREFIJO."coloquium WHERE id".DB_PREFIJO."coloquium= ".$_GET['id']."";
    ...
    if(isset($_POST['buscar'])){
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."coloquium WHERE 1 ".$busqueda."");
        
        if($cantidad != 0){
            $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
            mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
            $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."coloquium WHERE 1 ".$busqueda." ORDER BY id".DB_PREFIJO."coloquium
        ...
        $cantidad = cantidadRegistros("SELECT * FROM ".DB_PREFIJO."coloquium WHERE 1");
        
        $cat=new pagination($cantidad,25,$current_page,5 /*number of button*/);
        mysqli_query($conexion,"SET lc_time_names = 'es_MX'" );
        $registro = "SELECT *,date_format(creado, '%d-%m-%Y') as creado FROM ".DB_PREFIJO."coloquium  ORDER BY id".DB_PREFIJO."coloquium DESC LIMIT $cat->Start , $cat->End";
        $resultado = mysqli_query($conexion,$registro);
		...
		<div class="container">
			<div class="sixteen columns">
				<div class="list-items">
					<div class="titulo">
						<h4>Noticias</h4>
						<p>Lista de todas los noticias</p>
					</div>
	        			...
	        			<?php if($cantidad != 0){ ?>
	        				...
	        				?>
			        			<tr>
			        				...
			        				<?php if($_SESSION[PREFIJO.'tipo'] == 1){ ?>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/editar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'coloquium']; ?>/">Editar</a></td>
			        				<td><a href="<?php echo ADMINURL; ?>content/<?php echo $_GET['do']; ?>/eliminar/<?php echo $rowPublicacion['id'.DB_PREFIJO.'coloquium']; ?>/" class="confirm" title="¿Está seguro de borrar este registro?" >Eliminar</a></td>
			        				<?php } ?>
			        			</tr>
		        			<?php } ?>
	        			<?php }else{ ?>
